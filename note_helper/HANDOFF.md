...
(design + verified API citations + decisions), `README.md` (user-facing status),
and `JOURNAL.md` (past-session narratives). This file = current state + how to continue.

Last updated: 2026-06-21 (doc reorg 2026-07-01).

## Current state (2026-06-21)

Live-verified on IC6.1.8, all committed + pushed to `main` (HEAD `39433e6`). The tool turns
tables into schematic note annotations (loose shapes / symbol / resizable pcell) **and** imports
SVG vector art, with **live drag-ghost placement**, a **tabbed Table/SVG form** with file Browse,
and **Markdown file I/O** on both the SKILL and Python sides. Feature/milestone detail is in
**§ Where we are** below; the two most recent session narratives (M4 batch, then the M5-UX
GUI/placement rework) are archived in `JOURNAL.md`. Gotchas from those sessions live in
auto-memory ([[reference_skill_tabfield_access]], [[reference_skill_interactive_defer]],
[[reference_python_tkinter_env]]) and REQUIREMENTS §6/§7/§8.

**Open / next:** M5 (Excel import) + optional DXF input — see **§ Next step**. No blockers.
