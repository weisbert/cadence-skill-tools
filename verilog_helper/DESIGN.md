# verilog_helper — DESIGN (durable architecture, decisions, env recipes)

**Edited in place, organized by topic — NOT append-only.** When a decision changes, edit
the section (or move the old text under a "Superseded" note); do not stack a new block.
Current status + resume-here = `HANDOFF.md`; session-by-session history = `JOURNAL.md`.
...
## 8. Where the durable knowledge lives (pointers)
- **This file (`DESIGN.md`)** — architecture, stage decisions, env recipes, locked decisions.
- **`JOURNAL.md`** — session-by-session history + root-cause dossiers (LPBT/WuR bring-up).
- **`examples/<dut>/SPEC_CHECKLIST.md`** — authoritative per-DUT truth-table + divide/duty/pwsel/ADCDIV laws.
- **`../Verilog_check/Xcelium_安装交接.md`** (+ `setup_xcelium.sh`) — the Xcelium install/env handoff (see §4).
- **Claude Code auto-memory** (this project): `reference_cadence_config_nesting` (the #28
  cross-view / nested-config root), `reference_cadence_tcltk_env_poison`,
  `feedback_virtuoso_external_env_debug`.
  (The old bare slugs `xcelium-working-and-gotchas` / `oa2verilog-rhel8-recipe` /
  `verilog-check-project` / `eda-install-layout` were never created in any memory dir here —
  stale references, removed 2026-07-01.)
